...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Error fetching tasks by employee ID", error });
      }
    }
  );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Error fetching tasks by status", error });
      }
    }
  );
        ...
        try {
          ...
          if (employees.length > 0) {
            ...
            const response = await axios.post(
              "http://localhost:8000/api/algorithm/allocate-tasks",
              {
            ...
            if (allocations && allocations.length > 0) {
              ...
              for (const employee of updatedEmployees) {
                await storage.updateEmployeeWorkload(
                  employee.id,
  ...
  app.patch("/api/task/update/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        updatedTask.assigned_employee_id
      ) {
        ...
        if (employee) {
          const newWorkload = Math.max(
            0,
            employee.current_workload - updatedTask.estimated_effort
          );
  ...
  app.post("/api/assign/task", async (req: Request, res: Response) => {
    try {
      ...
      const newWorkload =
        employee.current_workload + (task.estimated_effort || 1);
      await storage.updateEmployeeWorkload(employeeId, newWorkload);

      res.json({
        task: updatedTask,
        employee: { ...employee, current_workload: newWorkload },
      });
    } catch (error) {
  ...
  app.post("/api/assign/tasks", async (_req: Request, res: Response) => {
    try {
      ...
      const unassignedTasks = allTasks.filter(
        (task) =>
          task.status !== TaskStatus.COMPLETED && !task.assigned_employee_id
      );

      console.log("Unassingen");
      ...
      if (employees.length === 0) {
        return res
          .status(400)
          .json({ message: "No employees available for assignment" });
      }
  ...
  app.post("/api/balance/workload", async (_req: Request, res: Response) => {
    try {
      ...
      if (employees.length < 2) {
        return res
          .status(400)
          .json({ message: "Need at least 2 employees to balance workload" });
      }

      // Call the Python API for workload balancing
      const response = await axios.post(
        "http://localhost:8000/api/algorithm/balance-workload",
        {
          tasks: activeTasks,
          employees: employees,
        }
  ...
  app.post("/api/urgent/task", async (req: Request, res: Response) => {
    try {
      ...
      if (employees.length === 0) {
        return res
          .status(400)
          .json({ message: "No employees available for urgent task" });
      }

      // Call the Python API for urgent task handling
      const response = await axios.post(
        "http://localhost:8000/api/algorithm/handle-urgent",
        {
          urgent_task: task,
          tasks: allTasks,
          employees: employees,
